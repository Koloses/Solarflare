...
// WiVRn addition (not in the upstream reference) that run the wavelet lifting in
// float16_t instead of fp32. The reference always uses fp32 math (with fp16 *storage*
// at precision 1). fp16 math loses precision on small high-frequency coefficients,
// which would corrupt motion/detail while leaving static (low-frequency) content
// intact. When this env var is set, the encoder loads the fp32-math DWT shader even
// on fp16-capable GPUs, matching the reference's precision-1 behaviour.
static inline bool enc_no_fp16_math()
{
	static const bool v = std::getenv("PYROWAVE_NO_FP16_MATH") != nullptr;
	return v;
}
        ...
        ds_pool(make_descriptor_pool(device))
{
	...
	{
		auto ef = phys_dev.getFeatures2<vk::PhysicalDeviceFeatures2, vk::PhysicalDevice8BitStorageFeatures, vk::PhysicalDeviceShaderFloat16Int8Features, vk::PhysicalDeviceSubgroupSizeControlFeatures>();
		...
		const auto & fsg = ef.get<vk::PhysicalDeviceSubgroupSizeControlFeatures>();
		...
		if (!feat13.computeFullSubgroups) feat13.computeFullSubgroups = fsg.computeFullSubgroups;
		if (!feat13.subgroupSizeControl) feat13.subgroupSizeControl = fsg.subgroupSizeControl;
	}
	...
	// block_packing pipeline
	{
		...
		psi.set_subgroup_size(prop13, info, 4, enc_cap_max_log2(6));
	...
	// quant pipeline
	{
		...
		pipeline_subgroup_info psi;
		psi.set_subgroup_size(prop13, info, 3, enc_cap_max_log2(7));
	...
	// dwt pipeline
	{
		...
		bool use_fp16_math = feat12.shaderFloat16 && !enc_no_fp16_math();
		auto shader = load_shader(device, std::string("dwt_" XSTR(PYROWAVE_PRECISION)) + (use_fp16_math ? "_fp16" : ""));
		if (enc_no_fp16_math())
			std::cerr << "PyroWave: forcing fp32-math DWT (dwt_" XSTR(PYROWAVE_PRECISION) ")" << std::endl;
		vk::ComputePipelineCreateInfo info{
		        .stage = {
		                .flags = vk::PipelineShaderStageCreateFlagBits::eRequireFullSubgroups,
		                .stage = vk::ShaderStageFlagBits::eCompute,
		...
		psi.set_subgroup_size(prop13, info, 2, enc_cap_max_log2(7));
...
Encoder::ValidationStats Encoder::validate_frame(const void * mapped_meta, const void * mapped_bitstream) const
{
	ValidationStats s;
	auto * meta = static_cast<const BitstreamPacket *>(mapped_meta);
	auto * bitstream_u32 = static_cast<const uint32_t *>(mapped_bitstream);
	s.total_blocks = uint32_t(block_count_32x32);
	for (int i = 0; i < block_count_32x32; i++)
	{
		if (meta[i].num_words == 0)
			continue;
		s.nonzero_blocks++;
		s.max_num_words = std::max<uint32_t>(s.max_num_words, meta[i].num_words);
		if (!validate_bitstream(bitstream_u32, meta, i))
			s.invalid_blocks++;
	}
	return s;
}
